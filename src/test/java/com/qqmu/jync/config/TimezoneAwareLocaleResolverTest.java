...
class TimezoneAwareLocaleResolverTest {
    ...
    private static final String LANG_COOKIE = TimezoneAwareLocaleResolver.LANG_COOKIE;
    ...
    /** Upgrades from SyncTool: the legacy language cookie still carries the user's choice. */
    @Test
    void legacyLanguageCookieIsHonouredWhenNewCookieAbsent() {
        TimezoneAwareLocaleResolver resolver = resolver();
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addPreferredLocale(Locale.US);
        request.setCookies(new Cookie(TimezoneAwareLocaleResolver.LEGACY_LANG_COOKIE, "zh-CN"));

        assertThat(resolver.resolveLocale(request)).isEqualTo(Locale.SIMPLIFIED_CHINESE);
    }

    /** Once the renamed cookie exists it wins, so a post-upgrade switch is not overridden. */
    @Test
    void newLanguageCookieBeatsLegacyOne() {
        TimezoneAwareLocaleResolver resolver = resolver();
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setCookies(
                new Cookie(LANG_COOKIE, "en"),
                new Cookie(TimezoneAwareLocaleResolver.LEGACY_LANG_COOKIE, "zh-CN"));

        assertThat(resolver.resolveLocale(request)).isEqualTo(Locale.ENGLISH);
    }

    /**
     * A malformed legacy cookie must degrade, not 500. {@code zh_CN.UTF-8} is an ordinary
     * POSIX locale string, but {@code StringUtils.parseLocale} rejects the dot — and the
     * base class's invalid-cookie tolerance does not wrap the legacy fallback path.
     */
    @Test
    void malformedLegacyLanguageCookieFallsBackInsteadOfThrowing() {
        TimezoneAwareLocaleResolver resolver = resolver();
        resolver.setRejectInvalidCookies(false);
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addPreferredLocale(Locale.US);
        request.setCookies(new Cookie(TimezoneAwareLocaleResolver.LEGACY_LANG_COOKIE, "zh_CN.UTF-8"));

        assertThat(resolver.resolveLocale(request)).isEqualTo(Locale.ENGLISH);
    }

    /** The browser keeps reporting the timezone under the legacy name until theme.js rewrites it. */
    @Test
    void legacyTimezoneCookieIsHonouredWhenNewCookieAbsent() {
        TimezoneAwareLocaleResolver resolver = resolver();
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addPreferredLocale(Locale.US);
        request.setCookies(new Cookie(TimezoneAwareLocaleResolver.LEGACY_TZ_COOKIE, "Asia/Shanghai"));
