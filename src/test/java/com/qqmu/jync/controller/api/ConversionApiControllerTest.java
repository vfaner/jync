 ...
 * {@code com.qqmu.jync.config.SecurityConfigTest}. Without this the security filter chain answers
...
class ConversionApiControllerTest {
    ...
    void aPassingCheckReportsTheTempName() throws Exception {
        when(assistService.validate(1L, "GET_TOTAL", "CREATE PROCEDURE GET_TOTAL() BEGIN END"))
                .thenReturn(passed("JYNC_AI_CHECK_ABC", List.of()));
        ...
        mvc.perform(post("/api/projects/1/conversions/validate").with(csrf())
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.tempName").value("JYNC_AI_CHECK_ABC"));
    }
    ...
    void aFailingCheckReportsTheEnginesError() throws Exception {
        when(assistService.validate(anyLong(), anyString(), anyString()))
                .thenReturn(rejected("Table \"NOPE\" not found", "JYNC_AI_CHECK_ABC"));
    ...
    @Test
    @DisplayName("a failed check the target gave no message for must not claim OK")
    void aFailedCheckWithoutMessageFallsBackToAKey() throws Exception {
        when(assistService.validate(anyLong(), anyString(), anyString()))
                .thenReturn(rejected(null, "JYNC_AI_CHECK_ABC"));

        // Some SQLExceptions carry no message; "OK" next to success:false made the detail
        // panel contradict its own red header, so the fallback is a translatable key.
        mvc.perform(post("/api/projects/1/conversions/validate").with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"kind\":\"VIEW\",\"name\":\"V\",\"sql\":\"CREATE VIEW V AS x\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("error.validate.unknown"));
    }

    @Test
    @DisplayName("a passing check without a message still says OK")
    void aPassingCheckKeepsTheOkMessage() throws Exception {
        when(assistService.validate(anyLong(), anyString(), anyString()))
                .thenReturn(passed("JYNC_AI_CHECK_ABC", List.of()));
