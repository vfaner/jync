...
public class PostgresDialect extends AbstractSqlDialect {
    ...
    private String buildWrapper(ProcedureMeta proc, String schema, String body) {
        ...
        // Dollar quoting avoids escaping anything inside the body — provided the tag itself
        // does not occur in the body, which would end the quoting early.
        String tag = uniqueDollarTag(body);
        sb.append(" LANGUAGE plpgsql AS $").append(tag).append("$\n")
                .append(body)
                .append("\n$").append(tag).append('$');
        return sb.toString();
    }

    /** A dollar-quote tag that does not occur in {@code body}; bumped until unique. */
    static String uniqueDollarTag(String body) {
        String tag = "jync";
        for (int n = 2; body.contains("$" + tag + "$"); n++) {
            tag = "jync" + n;
        }
        return tag;
    }
    ...
    @Override
    public String getDropIndexSql(com.qqmu.jync.dto.meta.IndexMeta index, String schema, String table) {
