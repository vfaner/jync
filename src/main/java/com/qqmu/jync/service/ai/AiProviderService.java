...
public class AiProviderService {
    ...
    /**
     * Used to keep the enable flip atomic WITHOUT holding a transaction across the HTTP
     * probe. A method-annotation @Transactional cannot do both: the probe must sit between
     * two short transactions, and self-invocation of a @Transactional method would bypass
     * the proxy anyway.
     */
     ...
     *
     * <p>The flip runs in its own short transaction and the probe runs OUTSIDE any
     * transaction: the HTTP call can take up to the provider's configured timeout, and
     * holding a database transaction (and its connection) across it would serialize every
     * other writer on this single-connection-pool H2 store for that long.
     */
    public AiProvider enable(Long id) {
        AiProvider saved = transactions.execute(status -> {
            if (repository.findById(id).isEmpty()) {
                throw new IllegalArgumentException("error.ai.provider.missing");
            }
            repository.disableAll();
            // Re-read: disableAll() was a bulk update, so the instance above may now be stale.
            AiProvider provider = repository.findById(id).orElseThrow();
            provider.setEnabled(true);
            AiProvider s = repository.save(provider);
            log.info("Enabled AI provider '{}'; all others switched off", s.getName());
            return s;
        });
        recordProbe(saved);
        return saved;
     ...
     *
     * <p>Deliberately not @Transactional: the probe is an HTTP call that can run up to the
     * provider's timeout, and the surrounding method must not hold a database transaction
     * (and its pooled connection) across it. The read and the write each get their own short
     * transaction from the repository instead.
     */
    ...
    /**
     * Runs the probe with no transaction open, then merges the outcome in a short one.
     * {@code provider} may be detached here; {@code save} reattaches it.
     */
