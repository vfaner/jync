...
**SyncTool 正式更名为 Jync（捷同）**，读作 "sync" —— J 代表 Java，捷同取「敏捷同步、两库皆同」之意。极简 4 字母生造词，全网无重名，从此好搜好记。在更名之外，本版还打包了全项目审查后的 27 项可靠性与缺陷修复（同步引擎、服务层、前端），见下文「审查修复」一节。
...
- **结构变更失败/跳过不再丢失变更**：此前 DDL 应用失败或被策略跳过时快照仍会刷新，检测器会认为「已处理」而永不重试；现在只有变更真正落地后才保存该表快照
- **锁租约心跳**：数据阶段每写一张表前校验租约，租约过期或被其他实例接管时立即中止本轮并记入变更日志，杜绝双实例并发写目标库
- **源库读不到正文的视图/存储过程不再误删**：源库权限抖动导致 body 读不到时曾被判定为「对象消失」，allowDrop 开启时会 DROP 掉目标库的真实对象；现在读不到 ≠ 消失
- **cursor 无法解析时回退全量重载**：不再带着读不懂的进度值静默按增量跑，造成数据缺口
- **纯主键表重投递幂等**：回放时的唯一键冲突按 no-op 处理，表不再被卡死
- **删除检测键值跨驱动规范化**：byte[]/decimal/时间类型在不同驱动下的渲染差异，不再把目标表整表误判为「源端已删除」而清空
- **truncate 不再静默失败**：TRUNCATE 与 DELETE 兜底都失败时报错，不会在未清空的表上继续写入

### 服务层与前端

- 项目更新不再重置启用状态、同步配置与创建时间；cron 表达式保存时校验
- 变更日志按保留天数每小时清理（`sync.changeLogRetentionDays`，默认 30 天，≤0 永久保留），设置页可见该取值
- 错误消息全面 i18n；删除连接前置存在性校验
- 前端：401 统一跳登录、非 JSON 响应兜底、按钮防双击、捐赠弹窗去重
