...
| **立即同步补跑** | 运行中的点击不丢弃：本轮结束后自动补跑一轮，连点多次合并为一轮 |
| **变更记录** | 每次变更的对象、类型、行数、耗时与错误详情 |
| **看板** | 项目数、连接数、同步表数、24 小时变更量与最近活动；概览与活动各限 5 条 |
| **分页** | 项目与变更日志列表分页，默认 20 条/页，可选 15/20/30/50/100，支持跳页与省略号就地展开 |
...
> 对比口径：下表基于各工具官方文档 / 开源版本的默认部署形态（截至 2026-09），仅针对「开箱即用的异构库持续同步」场景；各工具定位与强项不同，Jync 不适用场景见下文「什么时候**不**该用 Jync」。
...
| **持续增量同步** | 支持 轮询/Cron | 支持 日志级 | 支持 binlog | 支持 日志级 | 不适用（一次性批量定位） | 部分（需自建定时与增量逻辑） | 支持 触发器 | 不适用（一次性搬运定位） |
| **结构（DDL）同步** | 支持 **自动建表/索引/视图/存储过程** | 部分（输出 DDL 事件，落库需自写） | 部分（仅事件） | 部分（需自定义） | 不支持（需预建表） | 部分（手工映射） | 部分 | 支持（仅一次性） |
| **异构方言转换** | 支持 类型/函数/引号/分页/过程 | 不支持（定位为事件输出，转换由消费端负责） | 不支持（定位为 binlog 订阅，无方言转换） | 部分 | 部分（类型映射有限） | 部分（手工） | 部分 | 部分（一次性映射） |
| **国产数据库** | 支持 **达梦/金仓/OceanBase/TiDB/瀚高/海量/崖山/GBase/神通/OpenGauss** | 部分（无官方国产库 connector，需社区或自研） | 不支持（仅 MySQL 系，binlog 定位） | 部分（少数） | 部分（需自写插件） | 部分（靠通用 JDBC） | 部分 | 部分 |
| **源库侵入性** | **只读查询，零侵入** | 需开 binlog/wal + 复制权限 | 需开 binlog | 需开 binlog/wal | 只读 | 只读 | **需建触发器** | 只读 |
| **幂等 / 断点续传** | 支持 主键 upsert + 游标持久化 | 支持 offset | 支持 | 支持 checkpoint | 不适用（一次性批量） | 部分（需自建增量逻辑） | 支持 | 不适用（一次性搬运） |
| **可视化监控** | 支持 看板 + 变更日志 | 需接 Prometheus/Grafana | 需自建 | Flink UI（偏作业） | 日志 | 有限 | 有 Web 控制台 | 不适用（一次性搬运，无持续监控） |
| **内网离线** | 支持 无外网请求 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| **适用规模** | 中小规模、部门级、信创迁移 | 大规模流式 | MySQL 生态 | 大规模流式 | 大批量离线 | 复杂 ETL | 多主复制 | 临时搬数 |

> 图例：支持 = 开箱提供该能力　部分 = 部分支持或需额外工作　不支持 = 不提供该能力　不适用 = 维度不在该工具定位内
...
达梦、人大金仓、OceanBase、TiDB、瀚高、海量、崖山、南大通用、神通、OpenGauss 是预设的一等选项 —— 不是「通过通用 JDBC 也许能连上」，而是各自有专门的方言实现：`MERGE INTO ... FROM DUAL` 的 upsert 写法、类型上限（Oracle VARCHAR2 4000）、函数名差异、标识符引号规则都已处理。Oracle/SQL Server → 国产库的替换场景是本工具的主战场，而这恰恰是 Debezium、Canal 生态设计重心未覆盖的场景（其对国产库的官方支持相对有限）。
...
4. 点击「**立即同步**」验证一次（运行中再次点击会在本轮结束后自动补跑一轮），或点击「**启动同步**」开始持续轮询
5. 在「**变更日志**」查看每次同步的明细
...
「立即同步」与定时触发共用上面三层锁。点击时若锁已被占用，不再回一句冷冰冰的「正在运行」就完事 —— 请求会登记一次**补跑**：本轮结束、锁释放后，由后台线程自动再跑一轮。运行中点 N 次只合并出**一轮**补跑，不会排起 N 轮的队；补跑自己若又赶上锁被占（比如定时触发先抢到），会重新登记，直到消费掉为止。调度触发在运行中落下时走同一条路，因此「错过的这一轮」也会被补上。
...
- **分页窗口**：非法每页条数回落默认值、越界页码夹到最后一页；省略号只把自己那一侧展开 5 页、另一侧不动；选中的每页条数真的落到仓库查询上；只有一页时只渲染那一个页码
- **立即同步补跑**：运行中连点多次最终只多跑一轮；空闲时点击立即执行且不留队列；请求不存在的项目什么都不做
- **方言默认值**：MySQL 时间列的默认值必须带与列相同的小数精度（`CURRENT_TIMESTAMP(6)`），否则 CREATE TABLE 会被 MySQL 以 1067 拒绝
